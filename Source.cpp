...
void add(char** query, AVLTree& avlnode, String folder, RBTree& rb, int rows, int tree) {
    String key = query[1];
    String ref = query[1];
    String val;
    String folder2 = folder + '/';
    ...
    if (tree == 1) {
        avlnode.insertIterative(avlnode.root, ref, "");
        ofstream outFile(ref.constCharPointer(), ios::app);
        if (outFile)
        {
            outFile << "111" << ":" << data << endl;
            outFile.close();
        }
        else
        {
    ...
    {
        rb.insertNodeWithLineData(key, ref, data);
        ofstream outFile(ref.constCharPointer(), ios::app);
        if (outFile)
        {
            outFile << "111" << ":" << data << endl;
...
int main()
{
    // give here the path of the csv file which you are using 
    const char* filename = "C:/Users/HP/Desktop/3rd_sem_project/healthcare_dataset1.csv"; 
    AVLTree Node;
    Node.root = "NULL";
    RBTree rb;
    rb.root = "NULL";
